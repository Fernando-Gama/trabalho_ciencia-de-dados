...
(2002--2026), foi analisado no planejamento do tema, mas não foi usado
diretamente no processamento final. Ele possui estrutura mais voltada para
desempenho de seleções em torneios, enquanto o objetivo deste trabalho exige uma
base em nível de partida. Por isso, foi construído um dataset derivado a partir
de duas fontes principais: resultados internacionais de seleções e ranking FIFA.

A base de resultados internacionais foi utilizada porque contém datas, seleções,
placares, torneios e informação de campo neutro, permitindo identificar partidas
da Copa do Mundo e calcular o histórico pré-Copa de cada seleção. Já a base de
ranking masculino da FIFA foi utilizada porque fornece uma medida externa de
força relativa das seleções antes do torneio. A combinação das duas bases tornou
possível criar atributos anteriores à Copa e prever o resultado de cada jogo sem
usar informações futuras. A edição de 2026 não foi usada porque ainda não possui
resultados finais completos, isto é, não há rótulos reais para treinamento ou
avaliação.
...
\subsection{Prevenção de Vazamento Temporal}
...
\subsection{Engenharia e Seleção de Atributos}
...
\subsection{Testes Estatísticos das Hipóteses}

Além da comparação preditiva, foram executados testes estatísticos para avaliar
associação entre atributos e resultado. Foram usados qui-quadrado de
independência para variáveis categorizadas e correlação de Spearman para
atributos numéricos. O resultado foi convertido para escala direcional, em que
vitória da Seleção A vale 1, empate vale 0 e vitória da Seleção B vale -1.

A escolha do qui-quadrado se justifica porque o alvo do problema é categórico
(vitória da Seleção A, empate ou vitória da Seleção B) e algumas hipóteses foram
avaliadas por faixas ou categorias, como diferença de ranking e vantagem de forma
recente. Esse teste permite verificar se a distribuição dos resultados muda de
forma relevante entre os grupos. Como complemento, foi usado o V de Cramer para
medir o tamanho do efeito, pois o p-valor indica existência de associação, mas
não informa sua intensidade.

A correlação de Spearman foi utilizada para atributos numéricos porque ela mede
associação monotônica sem exigir normalidade dos dados. Essa escolha é adequada
para variáveis como diferença de ranking, diferença de pontos FIFA e diferenças
de aproveitamento, que podem não seguir distribuição normal e podem ter relações
não lineares com o resultado. Assim, os testes estatísticos complementam a
modelagem preditiva: eles indicam associação entre variáveis e resultado, mas
não afirmam causalidade.

\begin{table}[H]
\centering
\caption{Resumo dos testes estatísticos das hipóteses.}
\label{tab:testes_hipoteses}
...
Os testes estatísticos reforçam a conclusão de que ranking FIFA, forma recente
e desempenho no ciclo pré-Copa possuem associação com o resultado das partidas.
O maior efeito foi observado em H1, confirmando o ranking como o fator mais
consistente. A H4 não apresentou evidência forte no teste de qui-quadrado,
embora a correlação de Spearman para força dos adversários tenha sido
estatisticamente significativa, mas fraca. Portanto, essa hipótese deve ser
interpretada com mais cautela.
